...
class CircleCommand:
    ...
    def __str__(self):
        return '<Command radius="' + str(self.radius) + '" width="' + \
               str(self.width) + '" color="' + \
            self.color + '">Circle</Command>'


class WriteCommand:
    def __init__(self, phrase, size, chosenFont, color="black"):
        self.phrase = phrase
        self.size = size
        self.color = color
    ...
    def draw(self, turtle):
        turtle.pencolor(self.color)
        turtle.write(self.phrase, font=(
            str(self.font), int(self.size), "normal"))

    def __str__(self):
        return '<Command phrase="' + str(self.phrase) + '" size="' + \
               str(self.size) + '" font="' + str(self.font) + \
            '" color="' + self.color + '">Write</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            graphicsCommandsElement = xmldoc.getElementsByTagName("GraphicsCommands")[
                0]
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    phrase = attr["phrase"].value
                    size = float(attr["size"].value)
                    font = attr["font"].value
                    color = attr["color"].value.strip()
                    cmd = WriteCommand(phrase, size, font, color)
        ...
        def circleHandler():
            ...
            cmd = CircleCommand(float(radiusSize.get()), float(
                widthSize.get()), penColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
        ...
        phraseLabel = tkinter.Label(sideBar, text="Phrase to Write")
        phraseLabel.pack()
        phrase = tkinter.StringVar()
        phraseEntry = tkinter.Entry(sideBar, textvariable=phrase)
        phrase.set(str("CS360!"))
        phraseEntry.pack()

        sizeLabel = tkinter.Label(sideBar, text="Point Size")
        sizeLabel.pack()
        size = tkinter.StringVar()
        sizeEntry = tkinter.Entry(sideBar, textvariable=size)
        size.set("10")
        sizeEntry.pack()
        
        fontLabel = tkinter.Label(sideBar, text="Font Type")
        fontLabel.pack()
        fontList = font.families()
        fontType = tkinter.StringVar()
        fontType.set("Arial")
        fontMenu = tkinter.OptionMenu(sideBar, fontType, *fontList)
        fontMenu.pack()

        def writeHandler():
            cmd = WriteCommand(phrase.get(), float(
                size.get()), str(fontType.get()), penColor.get())
            cmd.draw(theTurtle)
        ...
        writeButton = tkinter.Button(
            sideBar, text="Write Text", command=writeHandler)
        writeButton.pack(fill=tkinter.BOTH)
