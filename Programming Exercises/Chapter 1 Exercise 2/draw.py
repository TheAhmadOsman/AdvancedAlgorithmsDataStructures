...
class StarCommand:
    def __init__(self, size, width=1, color="black"):
        self.size = size
        self.width = width
        self.color = color
        self.angle = 120
    ...
    def draw(self, turtle):
        ...
        for side in range(5):
            ...
            turtle.forward(self.size)
            turtle.right(72 - self.angle)

    def __str__(self):
        return '<Command size="' + str(self.size) + '" width="' + \
               str(self.width) + '" color="' + \
            self.color + '">Star</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    width = float(attr["width"].value)
                    color = attr["color"].value.strip()
                    cmd = StarCommand(size, width, color)
        ...
        sizeLabel = tkinter.Label(sideBar, text="Size")
        sizeLabel.pack()
        ...
        def starHandler():
            ...
            # the draw method. Adding the command to the graphicsCommands sequence means the
            # application will remember the picture.
            cmd = StarCommand(float(size.get()), float(
                widthSize.get()), penColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
        ...
        # This creates the button widget in the sideBar. The fill=tkinter.BOTH causes the button
        # to expand to fill the entire width of the sideBar.
        starButton = tkinter.Button(
            sideBar, text="Draw Star", command=starHandler)
